...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id'])) {
    ...
    $stmt = $conn->prepare("UPDATE notes_requests SET status = ? WHERE id = ?");
    if (!$stmt) {
...
$pendingRequests = $conn->query("SELECT * FROM notes_requests WHERE status = 'pending' ORDER BY uploaded_at DESC");
$approvedPYQs = $conn->query("SELECT * FROM notes_requests WHERE status = 'approved' ORDER BY uploaded_at DESC");
?>
