...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (move_uploaded_file($file['tmp_name'], $upload_path)) {
        ...
        $stmt = $connection->prepare("INSERT INTO notes_requests (subject, file_path, status, uploaded_at) VALUES (?, ?, ?, ?)");
        if (!$stmt) {
            alertAndRedirect('error', 'Database Error', 'Prepare statement failed: ' . $connection->error);
        }
        $stmt->bind_param("ssss", $subject,  $upload_path, $status, $submitted_at);
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            $connection->close();
            alertAndRedirect('error', 'Database Error', 'Failed to save Notes submission: ' . $connection->error);
        }
    } else {
