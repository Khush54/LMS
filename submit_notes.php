...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (move_uploaded_file($file['tmp_name'], $upload_path)) {
        ...
        $stmt = $conn->prepare("INSERT INTO notes_requests (subject, file_path, status, uploaded_at) VALUES (?, ?, ?, ?)");
        if (!$stmt) {
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            $conn->close();
            alertAndRedirect('error', 'Database Error', 'Failed to save Notes submission: ' . $conn->error);
        }
    } else {
